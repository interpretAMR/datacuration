...
afp <-read_tsv("ATB_Enterobacter_AFP.tsv.gz") %>% filter(HQ)
denominator <- afp %>% nrow()
...
ast<-read_delim("Enterobacter_AST.tsv.gz") %>% rename(BioSample=`#BioSample`)
...
cip_quinolone <- solo_marker_atb(ast_matched = ast_eh, afp_matched=afp_eh %>% filter(`Gene symbol`!="oqxB"), antibiotic = "ciprofloxacin", refgene_subclass = c("PHENICOL/QUINOLONE", "QUINOLONE"))

cip_quinolone$solo_stats

# excluding oqxAB as this is core
cip_quinolone_noOqx <- solo_marker_atb(ast_matched = ast_eh, afp_matched=afp_eh %>% filter(!(`Gene symbol` %in% c("oqxB", "oqxA"))), antibiotic = "ciprofloxacin", refgene_subclass = c("PHENICOL/QUINOLONE", "QUINOLONE"))
...
``` {r}
cip_binary <- getBinMatrix(ast_matched = ast_eh, afp_matched = afp_eh %>% filter(!(`Gene symbol` %in% c("oqxB", "oqxA"))), antibiotic = "ciprofloxacin", refgene_subclass = c("PHENICOL/QUINOLONE", "QUINOLONE"), maf=5)
